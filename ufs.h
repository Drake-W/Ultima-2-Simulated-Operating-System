...
class ufs{
	...
	struct i_node{
		...
		int size;										//file size indicates the actual file size not the number of blocks used by this file
		int permission[4] = { 1, 1, 0, 0};				//rwrw(owner, others) feel free to implement this as a bit pattern
		unsigned int blocks[4] = { 0, 0, 0, 0 };		//array of integers (block #) belonging to this file (max file size = 4x128 bytes) 
														//you may also try to use a bitmap to represent the used blocks, for example a 16 bit 
														//representation showing which blocks are used by  which i node
...
//File operations
//------------------------------------------------------------------------------
	int Open(int T_id,int callingtask, char* filenm,char* mode);		/* file handle must be valid and belong to t-id or file name should be
...
//------------------------------------------------------------------------------
	...
	void dump( WINDOW * win);							// dump the ufs inodes as well as the ufs data blocks
